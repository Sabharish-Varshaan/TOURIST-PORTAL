...
export class WebRTCService {
  ...
  /** Caller may send an offer before callee clicks Accept — buffer until we have mic + peerConnection */
  ...
  async acceptCall(audioOnly: boolean = true): Promise<void> {
    try {
      ...
      if (!this.peerConnection) {
        ...
        // Offer may have arrived over WebSocket before Accept (getUserMedia)
  ...
  async handleSignal(signal: any): Promise<void> {
    try {
      ...
      if (signal.type === "offer" && signal.sdp) {
        if (!this.localStream) {
          this.pendingOfferSdp = signal.sdp
          return
        }
        if (!this.peerConnection) {
          this.callbacks.onError?.(new Error("No peer connection; accept the call first"))
          return
        }
