...
export default function TouristDualChat({ touristId }: TouristDualChatProps) {
  ...
  const authorityCallIdRef = useRef<string | null>(null)
  const responderCallIdRef = useRef<string | null>(null)
  const authorityIncomingRtcRef = useRef<((payload: unknown) => void) | null>(null)
  const responderIncomingRtcRef = useRef<((payload: unknown) => void) | null>(null)

  useEffect(() => {
    authorityCallIdRef.current = authorityCallId
  }, [authorityCallId])
  useEffect(() => {
    responderCallIdRef.current = responderCallId
  }, [responderCallId])
  ...
  const {
    ...
    onWebRTCSignal: (callId, signal) => {
      if (callId === authorityCallIdRef.current) authorityIncomingRtcRef.current?.(signal)
    },
  })
  ...
  useEffect(() => {
    const enhancedHandler = (data: Record<string, unknown>) => {
      ...
      if (data.type === "incoming_call") {
        ...
        authorityCallIdRef.current = cid
        setAuthorityCallId(cid)
        ...
        authorityCallIdRef.current = null
  ...
  const {
    ...
    onWebRTCSignal: (callId, signal) => {
      if (callId === responderCallIdRef.current) responderIncomingRtcRef.current?.(signal)
    },
  })
  ...
  useEffect(() => {
    const enhancedHandler = (data: Record<string, unknown>) => {
      ...
      if (data.type === "incoming_call") {
        const cid = data.call_id as string
        responderCallIdRef.current = cid
        setResponderCallId(cid)
        ...
        responderCallIdRef.current = null
  ...
  return (
    <>
      ...
      <CallDialog
        ...
        incomingSignalRef={authorityIncomingRtcRef}
        onSignal={(signal) => {
          if (authorityCallId) {
      ...
      <CallDialog
        ...
        incomingSignalRef={responderIncomingRtcRef}
        onSignal={(signal) => {
          if (responderCallId) {
