...
export default function DashboardDualChat({ tourists }: DashboardDualChatProps) {
  ...
  const touristCallIdRef = useRef<string | null>(null)
  const responderCallIdRef = useRef<string | null>(null)
  const touristIncomingRtcRef = useRef<((payload: unknown) => void) | null>(null)
  const responderIncomingRtcRef = useRef<((payload: unknown) => void) | null>(null)

  useEffect(() => {
    touristCallIdRef.current = touristCallId
  }, [touristCallId])
  useEffect(() => {
  ...
  const {
    ...
    onWebRTCSignal: (callId, signal) => {
      if (callId === touristCallIdRef.current) touristIncomingRtcRef.current?.(signal)
    },
  })
  ...
  useEffect(() => {
    const enhancedHandler = (data: Record<string, unknown>) => {
      ...
      if (data.type === "incoming_call") {
        ...
        touristCallIdRef.current = cid
        setTouristCallId(cid)
