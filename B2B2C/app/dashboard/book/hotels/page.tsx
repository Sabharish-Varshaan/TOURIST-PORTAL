...
export default function BookHotelsPage() {
  ...
  const [results, setResults] = useState<{ id: string; name: string; price: number; rating?: string }[]>([]);
  ...
  const search = async () => {
    if (!location.trim() || !checkIn || !checkOut) {
      setError("Please fill location and dates.");
      return;
    }
    setError(null);
    setLoading(true);
    try {
      const res = await fetch(
        `/api/book/hotels?location=${encodeURIComponent(location)}&check_in=${encodeURIComponent(checkIn)}&check_out=${encodeURIComponent(checkOut)}`
      );
  ...
  const book = async (optionId: string) => {
    ...
    try {
      const res = await fetch("/api/book/hotels", {
        method: "POST",
      ...
      <header className="border-b border-[var(--border)] bg-[var(--bg-card)]/95 backdrop-blur sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-[var(--text-muted)]">Hotels</span>
          </div>
        </div>
      </header>

      <main className="max-w-4xl mx-auto px-4 py-8 flex-1">
        <h1 className="text-2xl font-bold text-[var(--text)]">Book hotels</h1>
        <p className="mt-1 text-[var(--text-muted)] text-sm">
          Book stays — linked to our SOS safety app. Add safety for this trip from the dashboard first.
        </p>
        ...
        <div className="mt-6 grid sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text)] mb-1">Check-in</label>
            <input
              type="date"
              value={checkIn}
          ...
          <div>
            <label className="block text-sm font-medium text-[var(--text)] mb-1">Check-out</label>
            <input
              type="date"
              value={checkOut}
            ...
            {results.map((opt) => (
              ...
              >
                <div>
                  ...
                  {opt.rating && <span className="ml-2 text-sm text-[var(--text-muted)]">★ {opt.rating}</span>}
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-semibold text-[var(--primary)]">₹{opt.price}/night</span>
                  {booked === opt.id ? (
