...
export default function BookTransportPage() {
  ...
  const [results, setResults] = useState<{ id: string; name: string; price: number; time: string }[]>([]);
  ...
  const search = async () => {
    ...
    try {
      const res = await fetch(
        `/api/book/transport?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${encodeURIComponent(date)}&mode=${mode}`
      );
  ...
  const book = async (optionId: string) => {
    ...
    try {
      const res = await fetch("/api/book/transport", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="border-b border-[var(--border)] bg-[var(--bg-card)]/95 backdrop-blur sticky top-0 z-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-[var(--text-muted)]">Bus & Train</span>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 flex-1">
        <ScrollReveal>
          <h1 className="text-2xl font-bold text-[var(--text)]">Book bus or train</h1>
          <p className="mt-1 text-[var(--text-muted)] text-sm">
            Book transport — linked to our SOS safety app. Add safety for this trip from the dashboard first.
          </p>
        </ScrollReveal>
        ...
        <div className="mt-6 flex gap-2">
          <button
            type="button"
            onClick={() => setMode("bus")}
            className={`rounded-[var(--radius)] px-4 py-2 text-sm font-medium ${mode === "bus" ? "bg-[var(--primary)] text-white" : "border border-[var(--border)] text-[var(--text)]"}`}
          >
            Bus
          </button>
          <button
            ...
            className={`rounded-[var(--radius)] px-4 py-2 text-sm font-medium ${mode === "train" ? "bg-[var(--primary)] text-white" : "border border-[var(--border)] text-[var(--text)]"}`}
          >
            Train
          </button>
        </div>
            ...
            {results.map((opt) => (
              ...
              >
                <div>
                  ...
                  <span className="ml-2 text-sm text-[var(--text-muted)]">{opt.time}</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-semibold text-[var(--primary)]">₹{opt.price}</span>
                  {booked === opt.id ? (
